...
//	IIC_WR_Byte((addr << 1) | 0);
    IIC_WR_Word(((0xd << 8) | 0) | reg);  //发送器件地址和写指令以及寄存器地址
	for(i = 0; i < len;i++)
	{
	    IIC_WR_Byte(buf[i]);    //写入指令
	}
	return 0;
//	MPU_IIC_Start();
...
u8 MPU_Write_Byte(u8 reg, u8 data)
{
    ...
    USCI_B_I2C_setSlaveAddress(USCI_B0_BASE, MPU_ADDR);
    IIC_WR_Word((reg << 8) | data);
    return 1;
//    MPU_IIC_Start();
...
u8 MPU_Read_Byte(u8 reg)
{
	...
	USCI_B_I2C_setSlaveAddress(USCI_B0_BASE, MPU_ADDR);
//	TxData = (OLED_WriteData_Addr << 8) | reg;
	IIC_WR_Word(((MPU_ADDR << 8)) | reg);
//
    //IIC_WR_Byte(MPU_ADDR);    //发送器件地址+写命令
//	IIC_WR_Byte(reg);                //写寄存器地址
	//IIC_WR_Byte((MPU_ADDR << 1) | 1);       //发送器件地址+读命令
	//吗                                                                                    `    USCI_B_I2C_setSlaveAddress(USCI_B0_BASE, MPU_ADDR);
	IIC_RD_Byte(reg , &res);

//	IIC_RD_Bytes(MPU_ADDR >> 1 + 1, 3, &res);
	/*MPU_IIC_Start();
	MPU_IIC_Send_Byte((MPU_ADDR << 1) | 0); //发送器件地址+写命令
	MPU_IIC_Wait_Ack();						//等待应答
	MPU_IIC_Send_Byte(reg);					//写寄存器地址
